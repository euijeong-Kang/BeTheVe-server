...
public class CustomEmailPasswordAuthProvider implements AuthenticationProvider {
    ...
    protected void additionalAuthenticationChecks(UserDetails userDetails,
                                                  CustomEmailPasswordAuthToken authentication) throws BusinessException {
        ...
        if (authentication.getCredentials() == null) {
            ...
            throw new BusinessException("패스워드 없음");
        }
        String presentedPassword = authentication.getCredentials().toString();
        log.debug("authentication.presentedPassword = {}",presentedPassword);

        if (!this.passwordEncoder.matches(presentedPassword, userDetails.getPassword())) {
            throw new BusinessException("잘못된 패스워드");
        }
    }

    @Override
    public Authentication authenticate(Authentication authentication) throws BusinessException {
    ...
    protected final UserDetails retrieveUser(String username) throws BusinessException {
        try {
